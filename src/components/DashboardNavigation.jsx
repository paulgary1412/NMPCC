...
import "../assets/DashboardNavigation.css";  // Updated import
  ...
  return (
    ...
    >
      ...
      <span className={isCollapsed ? "collapsed" : ""}>
        {isCollapsed ? (
          <FaDashcube size={60} aria-label="Dashboard Icon" />
        ) : (
          <span>Dashboard</span>
        )}
      </span>
      ...
      <button onClick={() => setActiveSection("products")} aria-label="Products">
        {isCollapsed ? (
          <FaShoppingCart size={40} aria-label="Products Icon" />
        ) : (
          "Products"
        )}
      </button>
      ...
      <button onClick={() => setActiveSection("packages")} aria-label="Packages">
        {isCollapsed ? (
          <FaBox size={40} aria-label="Packages Icon" />
        ) : (
          "Packages"
        )}
      </button>
      ...
      <button onClick={() => setActiveSection("AdminOrders")} aria-label="Orders">
        {isCollapsed ? (
          <FaClipboardList size={40} aria-label="Orders Icon" />
        ) : (
          "AdminOrders"
        )}
      </button>
