...
const AdminOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get("http://192.168.1.45:5000/checkout/");
  ...
  const handleUpdate = async (orderId, newStatusOrder, newStatusInfo) => {
    try {
      const response = await axios.put(`http://192.168.1.45:5000/checkout/update/${orderId}`, {
  ...
  const handleDelete = async (orderId) => {
    try {
      await axios.delete(`http://192.168.1.45:5000/checkout/delete/${orderId}`);
